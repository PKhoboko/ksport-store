...
export default function CartClient() {
    ...
    const handleSuccess = async (reference: any) => {
        try {
            ...
            // 2. Send order straight to YOUR WhatsApp via Meta Cloud API — no customer action needed
            await sendOrderEmail({
                ref:           reference.reference,
                customerName,
                customerEmail:  "pwkhoboko@gmail.com",//user?.email ||
                customerPhone,
                items: cart.map(i => ({
                    name:     i.name,
                    price:    i.price,
